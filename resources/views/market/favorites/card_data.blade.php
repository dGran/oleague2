		...
		<div class="actions">
			<div class="dropdown dropleft">
				...
				<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
					...
					<a class="dropdown-item text-danger" href="" onclick="destroy_favorite('{{ $player->id }}', '{{ $player->season_player->player->name_addslashes() }}')">
						Eliminar favorito
					</a>
					@if (participant_of_user()->id != $player->season_player->participant_id)
						<div class="dropdown-divider"></div>
						<a class="dropdown-item {{ !$season->transfers_period || !$player->season_player->participant ? 'disabled' : '' }}" href="{{ route('market.trades.add', [$player->season_player->participant_id, $player->season_player->player->id]) }}">
							Abrir negociación
						</a>
						<a class="dropdown-item {{ !$season->clausules_period || !$player->season_player->participant || !$player->season_player->allow_clause_pay || ($player->season_player->participant && $player->season_player->participant->clauses_received_limit()) || ($player->season_player->participant && participant_of_user()->clauses_paid_limit()) || $player->season_player->participant && $player->season_player->participant->id == participant_of_user()->id || participant_of_user()->budget() < $player->season_player->clause_price() || participant_of_user()->max_players_limit() ? 'disabled' : '' }}" href="" onclick="pay_clause_player('{{ $player->season_player->id }}', '{{ $player->season_player->player->name_addslashes() }}', '{{ number_format($player->season_player->price, 2, ',', '.') }}')">
							Pagar claúsula
						</a>
						<a class="dropdown-item {{ !$season->free_players_period || $player->season_player->participant || participant_of_user()->max_players_limit() ? 'disabled' : '' }}" href="" onclick="sign_free_player('{{ $player->season_player->id }}', '{{ $player->season_player->player->name_addslashes() }}', '{{ number_format($player->season_player->season->free_players_cost, 2, ',', '.') }}')">
							Fichar jugador
						</a>
					@endif
				</div> {{-- dropdown-menu --}}
