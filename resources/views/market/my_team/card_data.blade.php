		...
		<div class="actions">
			<div class="dropdown dropleft">
				...
				<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
					...
					<div class="dropdown-divider"></div>
					<a class="dropdown-item {{ !active_season()->transfers_period || participant_of_user()->min_players_limit() ? 'disabled' : 'text-danger' }}" href="" onclick="dismiss_player('{{ $player->id }}', '{{ $player->player->name_addslashes() }}', '{{ number_format($player->season->free_players_remuneration, 2, ',', '.') }}')">
						Despedir
					</a>
				</div> {{-- dropdown-menu --}}
			</div> {{-- dropdown --}}
