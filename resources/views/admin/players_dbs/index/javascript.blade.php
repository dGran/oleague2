<script>
    ...
    function cancelFilters() {
        window.location.href = '{{ route("admin.players_dbs") }}';
    }
    ...
    $(".btn-delete").click(function(e) {
        ...
        if (allow_delete == 1) {
            swal({
                title: "¿Estás seguro?",
                text: 'Se va a eliminar la database "' + name + '". No se podrán deshacer los cambios!',
                buttons: {
                    confirm: {
            ...
            .then((value) => {
                if (value) {
                    ...
                    var url = form.attr('action').replace(':DATABASE_ID', id);
                    form.attr('action', url);
                    form.submit();
                }
            });
        } else {
            swal("La database tiene jugadores asociados por lo que no se puede eliminar.", {
                buttons: false,
                icon: "error",
                timer: 3000,
            });
    ...
    function destroyMany() {
        ...
        swal({
            title: "¿Estás seguro?",
            text: 'Se van a eliminar las databases seleccionadas (las databases con jugadores asociados no se eliminarán). No se podrán deshacer los cambios!.',
            buttons: {
                confirm: {
                    text: "Sí, estoy seguro",
                    value: true,
        ...
        .then((value) => {
            if (value) {
                ...
                var url = '{{ route("admin.players_dbs.destroy.many", ":ids") }}';
    ...
    function duplicateMany() {
        ...
        var url = '{{ route("admin.players_dbs.duplicate.many", ":ids") }}';
        url = url.replace(':ids', ids);
        window.location.href=url;
    }
    ...
    function export_file(type) {
        ...
        .then((value) => {
            if (value) {
                ...
                $(location).attr('href', 'databases_jugadores/exportar/' + filename + '/' + type + '/' + filterName + '/' + order);
            }
        });
    }
    ...
    function export_file_selected(type) {
        ...
        .then((value) => {
            if (value) {
                ...
                $(location).attr('href', 'databases_jugadores/exportar/' + filename + '/' + type + '/' + filterName + '/' + order + '/' + ids);
            }
        });
    }
