<script>
    ...
    function cancelFilters() {
        window.location.href = '{{ route("admin.seasons") }}';
    }
    ...
    $(".btn-delete").click(function(e) {
        ...
        if (allow_delete == 1) {
            swal({
                title: "¿Estás seguro?",
                text: 'Se va a eliminar la temporada "' + name + '". No se podrán deshacer los cambios!',
                buttons: {
                    confirm: {
            ...
            .then((value) => {
                if (value) {
                    ...
                    var url = form.attr('action').replace(':SEASON_ID', id);
                    form.attr('action', url);
                    form.submit();
                }
            });
        } else {
            swal("La temporada tiene xxxxxx asociados por lo que no se puede eliminar.", {
                buttons: false,
                icon: "error",
                timer: 3000,
            });
    ...
    function destroyMany() {
        ...
        swal({
            title: "¿Estás seguro?",
            text: 'Se van a eliminar las temporadas seleccionadas (las categorías con xxxxxx asociados no se eliminarán). No se podrán deshacer los cambios!.',
            buttons: {
                confirm: {
                    text: "Sí, estoy seguro",
                    value: true,
        ...
        .then((value) => {
            if (value) {
                ...
                var url = '{{ route("admin.seasons.destroy.many", ":ids") }}';
        ...
        .then((value) => {
            if (value) {
                ...
                if (!filename ) {
                    ...
                    var filename = 'temporadas_export' + time;
                }
                $(location).attr('href', 'temporadas/exportar/' + filename + '/' + type + '/' + filterName + '/' + order);
            }
        });
    }
    ...
    function export_file_selected(type) {
        ...
        .then((value) => {
            if (value) {
                ...
                $(location).attr('href', 'temporadas/exportar/' + filename + '/' + type + '/' + filterName + '/' + order + '/' + ids);
            }
        });
    }
