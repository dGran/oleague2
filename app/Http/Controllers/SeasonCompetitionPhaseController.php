...
class SeasonCompetitionPhaseController extends Controller
{
    ...
    {
        $competition = SeasonCompetition::where('slug','=', $slug)->firstOrFail();
        return view('admin.seasons_competitions_phases.add', compact('competition'));
    }
    ...
    public function save($slug)
    {
        ...
        $competition = SeasonCompetition::where('slug','=', $slug)->firstOrFail();
        $more_phases = SeasonCompetitionPhase::where('competition_id', '=', $competition->id)->count();

        $data = request()->all();
        $data['competition_id'] = $competition->id;
        $data['order'] = $more_phases + 1;
        $data['active'] = 0;
    ...
    public function edit($slug, $id)
    {
        ...
        if ($phase) {
            return view('admin.seasons_competitions_phases.edit', compact('phase', 'competition'));
        } else {
